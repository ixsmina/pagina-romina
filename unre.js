...
// --- Unreleased JavaScript ---

// Menú hamburguesa en móvil
const unreMenuToggle = document.getElementById('menu-toggle');
const unreMenu = document.getElementById('menu');

if (unreMenuToggle && unreMenu) {
  unreMenuToggle.addEventListener('click', () => {
    unreMenu.classList.toggle('show'); // Alterna la clase 'show' para mostrar/ocultar con CSS

    // Cambiar el icono del botón y el atributo aria-expanded
    if (unreMenu.classList.contains('show')) {
      unreMenuToggle.innerHTML = '×'; // Icono de cerrar (X)
      unreMenuToggle.setAttribute('aria-expanded', 'true');
    } else {
      unreMenuToggle.innerHTML = '☰'; // Icono de hamburguesa
      unreMenuToggle.setAttribute('aria-expanded', 'false');
    }
  });
}
